...
def log_validation(
    ...
    # breakpoint()
    print("Using GPU 1 for logging validation")
    ...
    pipeline = pipeline.to(dtype=torch.float16, device=device1)
    pipeline.set_progress_bar_config(disable=True)

    # run inference
    generator = torch.Generator(device=device1).manual_seed(args.seed) if args.seed else None
...
def encode_prompt(
    ...
    for i, (tokenizer, text_encoder) in enumerate(zip(clip_tokenizers, clip_text_encoders)):
        prompt_embeds, pooled_prompt_embeds = _encode_prompt_with_clip(
            ...
            text_input_ids=text_input_ids_list[i] if isinstance(text_input_ids_list, list) else None,
        )
    ...
    t5_prompt_embed = _encode_prompt_with_t5(
        ...
        prompt=prompt,
        num_images_per_prompt=num_images_per_prompt,
        text_input_ids=text_input_ids_list[:-1] if isinstance(text_input_ids_list, list) else None,
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            # print(epoch, args.validation_epochs)
